...
当前架构采用 **Web 前端 + Python HTTP API 后端** 的 B/S 模式，通过 Session 机制实现多标签页隔离和自动持久化。

```mermaid
graph TB
    subgraph "前端 Frontend"
        HTML[canmatrix_web_editor.html<br/>单页应用 SPA]
        LS[localStorage<br/>session_id 持久化]
        CLIP[内存剪贴板<br/>Ctrl+C/V/X 快捷键]
    end

    subgraph "HTTP API 层 api_server.py"
        ROUTER[aiohttp Router]
        SESSION_API[POST /api/session<br/>GET /api/session/{id}<br/>GET /api/sessions]
        CRUD_API[POST /api/messages<br/>PUT/DELETE /api/messages/{id}<br/>POST /api/signals/{msg_id}]
        IO_API[POST /api/import<br/>POST /api/export]
    end

    subgraph "会话管理层 session_manager.py"
        SM[SessionManager<br/>线程安全字典]
        S[Session<br/>id/file_path/db/timestamps]
    end

    subgraph "CLI 会话层 cli.py"
        CS[CanMatrixSession<br/>无头会话 + OpResult]
    end
    ...
    subgraph "外部依赖"
        ...
        AIOHTTP[aiohttp<br/>HTTP 框架]
        TOML_LIB[toml<br/>TOML 解析库]
    end

    HTML -->|fetch + X-Session-Id| ROUTER
    HTML --> LS
    HTML --> CLIP
...
```
canmatrix_web_editor.html (Web 前端)
  └── api_server.py (HTTP API 层)
        ├── session_manager.py (会话管理层)
        │     └── cli.py (CLI 会话层)
        │           └── core/ (核心数据 + 序列化层)
        └── cli.py (直接用于 I/O 操作)
...
- `session_manager.py` 包装 `cli.py` 的 `CanMatrixSession`，提供线程安全的会话池管理。
- `api_server.py` 通过 `X-Session-Id` header 路由到对应 Session，调用 `session_manager` 和 `cli.py`。
- 前端 `canmatrix_web_editor.html` 仅通过 REST API 与后端通信，不直接依赖任何 Python 模块。
...
```mermaid
flowchart LR
    subgraph "导入路径（打开本地文件）"
        FILE[本地文件<br/>TOML/JSON/XML/DBC]
        -->|POST /api/session{file_path}| API_S[api_server]
        -->|session_mgr.create| SM[SessionManager]
        -->|CanMatrixSession.load| CS_LOAD[cli.py]
        -->|I/O 模块| CD_NEW[CanDatabase 对象图]
    end

    subgraph "运行时编辑"
        WEB[Web 前端] -->|fetch + X-Session-Id| API_C[api_server CRUD]
        -->|session_mgr.get_session| S[Session.db]
        -->|每次变更后| AUTO_SAVE[session_mgr.save<br/>原子写入文件]
    end

    subgraph "导出路径"
        WEB_E[Web 前端] -->|POST /api/export| API_E[api_server]
        -->|session.db| EXPORT[CanMatrixSession.export_dbc]
        --> DBC_OUT[DBC 文件下载]
    end

    subgraph "会话恢复"
        WEB_R[Web 重开页面] -->|localStorage session_id| API_R[GET /api/session/{id}]
        -->|session_mgr.restore| RESTORE[从文件重建 CanMatrixSession]
    end
```

> **关键设计变更**：从 PyQt6 本地桌面应用重构为 Web 架构。核心变化：
> 1. **Session 绑定文件，非全局单例**：每个浏览器标签页通过 `session_id` 绑定独立文件，变更后自动落盘，无需手动保存。
> 2. **导入/导出改为打开/自动保存模式**：不再用"前端读文件→POST 内容→服务器解析"的搬运模式，而是由服务器直接操作文件系统。
> 3. **前端 localStorage 持久化 session_id**：页面重开时自动恢复上次编辑状态。
> 4. **原子写入**：`session_manager.py` 使用 `tmp + os.replace` 确保文件写入的原子性和数据安全。

### 2.4 Session Manager 设计

#### 2.4.1 核心数据结构

| 组件 | 说明 |
|------|------|
| `Session` | 单个会话对象，包含 `id`（UUID）、`file_path`（绑定的本地文件路径）、`db`（`CanMatrixSession` 实例）、`created_at`、`last_access` |
| `SessionManager` | 全局单例管理器，维护 `dict[session_id → Session]`，线程安全（`threading.Lock`） |

#### 2.4.2 SessionManager API

| 方法 | 说明 |
|------|------|
| `create(file_path=None)` | 创建新 Session。若提供文件路径则立即加载；否则创建空数据库。返回 `session_id` |
| `restore(session_id)` | 从文件重新加载 Session 的数据库，返回 `CanDatabase` |
| `save(session_id)` | 将 Session 的当前数据库写入绑定的文件（原子写入） |
| `get(session_id)` | 获取 Session 对象，自动更新 `last_access` |
| `destroy(session_id)` | 销毁 Session，释放资源 |
| `cleanup(timeout=1800)` | 清理超过 30 分钟未访问的过期 Session |

#### 2.4.3 文件命名约定

Session 绑定的 TOML 文件按 `{session_id}_{name}.canmatrix` 命名，存放于工作目录 `output/canmatrix_editor/` 下。

#### 2.4.4 自动落盘流程

每次 CRUD 端点（添加/修改/删除报文或信号）调用成功后，自动调用 `session_mgr.save(session_id)` 将当前内存中的 `CanDatabase` 序列化为 TOML 写入磁盘。前端无需调用保存 API，`doSave()` 已改为 no-op。
...
| 文件路径 | 大小 | 行数（估算） | 职责 |
|---------|------|-------------|------|
| `api_server.py` | 32 KB | ~1000 | Python HTTP API 服务器，基于 aiohttp，提供 Session/CRUD/IO 端点 |
| `canmatrix_web_editor.html` | 42 KB | ~1200 | 单页 Web 应用，提供完整 GUI 界面，支持快捷键和右键菜单 |
| `session_manager.py` | 8 KB | ~250 | `Session` 和 `SessionManager` 类，实现会话隔离和自动落盘 |
| `cli.py` | 10 KB | ~300 | `CanMatrixSession` + `OpResult`，CLI 无头会话层 |
| `main.py` | 1 KB | ~30 | 遗留 PyQt6 入口（已弃用，保留兼容性） |
| `core/__init__.py` | 0 KB | ~5 | 包初始化文件 |
| `core/can_database.py` | 8 KB | ~250 | `Signal`、`Message`、`CanDatabase` 数据模型 |
| `core/toml_io.py` | 4 KB | ~120 | TOML 导入/导出，稀疏输出，十六进制 ID |
| `core/json_io.py` | 1 KB | ~30 | JSON 导入/导出 |
| `core/xml_io.py` | 4 KB | ~130 | XML 导入/导出 |
| `core/dbc_io.py` | 6 KB | ~190 | DBC 导入/导出（依赖 cantools） |
| `gui/__init__.py` | 0 KB | ~5 | 包初始化文件 |
| `gui/main_window.py` | 21 KB | ~650 | `MainWindow`：菜单栏、工具栏、报文树、信号表 |
| `gui/widgets.py` | 9 KB | ~280 | `SignalTableModel`、`HexSpinBox` |
| `gui/message_editor.py` | 4 KB | ~130 | `MessageDialog`：报文添加/编辑对话框 |
| `gui/signal_editor.py` | 8 KB | ~250 | `SignalDialog`：信号添加/编辑对话框 |
| `test_per_message.py` | 2 KB | ~60 | 冒烟测试：per-message 模型、DBC 往返、CLI API |
...
| 统计项 | 数值 |
|--------|------|
| 有效源码文件数（不含 `__init__.py`） | 15 |
| 总代码行数（估算） | ~3500 行 |
| Python 平均文件大小 | ~7.5 KB |
| 测试文件数 | 1 |
| 测试代码行数（估算） | ~60 行 |
...
```mermaid
graph TD
    html[canmatrix_web_editor.html] -->|fetch REST API| api_server[api_server.py]

    api_server --> session_mgr[session_manager.py]
    api_server --> cli_py[cli.py]
...
| 文件 | 核心类 / 函数 |
|------|----------------|
| `api_server.py` | `ServerState`、`setup_routes(app)`、`handle_create_message`、`handle_export` 等路由处理函数 |
| `canmatrix_web_editor.html` | `api()`、`doNew()`、`doImport()`、`doExport()`、`initEditor()`、剪贴板快捷键处理 |
...
| `gui/main_window.py` | `MainWindow`（遗留 PyQt6 组件） |
| `gui/widgets.py` | `SignalTableModel`、`HexSpinBox`（遗留 PyQt6 组件） |
| `gui/message_editor.py` | `MessageDialog`（遗留 PyQt6 组件） |
| `gui/signal_editor.py` | `SignalDialog`（遗留 PyQt6 组件） |
