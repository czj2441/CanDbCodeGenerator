...
Storage layout (dotted keys only, NO tables/sections)::
...
def save_toml(database: CanDatabase, filepath: str) -> None:
    """Serialize a CanDatabase to a TOML file (dotted keys format)."""
    os.makedirs(os.path.dirname(filepath) or ".", exist_ok=True)
    content = database.to_toml_str()
...
def load_toml(filepath: str) -> CanDatabase:
    """Load a CanDatabase from a TOML file (dotted keys format)."""
