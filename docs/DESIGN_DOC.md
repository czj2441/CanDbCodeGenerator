...
- **TOML 主存储**：信号以 `[[messages.signals]]` 数组形式嵌套在报文下，增删信号只影响该报文块。
- **稀疏输出**：默认值的字段（`factor=1.0`、`offset=0.0` 等）在保存时省略，减少 diff 噪音。
- **十六进制 CAN ID**：TOML 中 `id = "0x100"`，与 CAN 工具链和硬件文档保持一致。
...
│  │  + 信号验证 (重叠/越界) + TOML/DBC 序列化        │
│  └─────────────────────────────────────┘            │
│                                                      │
│  ┌─────────────────────────────────────┐            │
│  │  core/  独立数据模型 + IO 层          │            │
│  │  can_database.py  toml_io.py        │            │
...
本地文件 → 前端 FileReader 读取内容
  → POST /api/import {format, content}
    → api_server._post_import()
      → CanDatabase.from_toml_str() / from_dict()
        → 替换当前 Session 的 db
          → 自动保存到 data/{session_id}_{name}.toml
```
...
Vue 组件点击导出 → Pinia store.exportDatabase()
  → POST /api/export {format: "dbc"|"toml"|"json"}
    → db.to_dbc_str() / to_toml_str() / to_dict()
      → 返回字符串内容 → 前端 Blob 下载
```
...
| `to_toml_str() → str` | 序列化为 TOML 字符串（稀疏输出） |
| `to_dbc_str() → str` | 导出为 DBC 格式字符串 |
| `from_toml_str(content) → CanDatabase` | 从 TOML 字符串反序列化 |
...
| 实现位置 | 方式 | 用途 |
|----------|------|------|
| `core/toml_io.py` → `save_toml()` | 手动逐行构造字符串 | CLI 离线保存，精确控制稀疏输出 |
| `api_server.py` → `CanDatabase.to_toml_str()` | `toml.dumps()` 库函数 | Web 运行时保存，过滤默认值字段 |
...
- TOML 输出时 CAN ID 使用 `"0x100"` 字符串格式（TOML 不支持 `0x` 前缀整数）
...
每个浏览器标签页对应一个独立的 **Session**，绑定一个磁盘 TOML 文件。所有变更自动落盘，浏览器崩溃后可通过 `localStorage` 中的 `session_id` 恢复。
...
- 兼容旧格式：`_find_session_file()` 同时查找 `.toml` 和 `.canmatrix` 后缀
...
- ✅ 注释以 `#` 书写，工程师可自由添加备注
- ⚠️ TOML 不支持 `0x` 前缀整数，CAN ID 需以字符串存储
...
- **权限需求**：仅需对项目目录和 `.toml`/`.dbc` 文件的读写权限，无需管理员权限

### 10.2 输入安全

| 格式 | 解析器 | 安全性 |
|------|--------|--------|
| TOML | `toml`（PyPI 纯 Python） | 无 C 扩展，无已知安全漏洞 |
...
| `core/toml_io.py` | ~115 | TOML 读写（手动构造，稀疏输出） |
