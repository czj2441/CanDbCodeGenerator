...
frontend/src/          (Vue 3 SPA)
  └── 通过 WebSocket ──→ app/server/lifecycle.py
        ├── app/services/session_manager.py  (会话管理)
        │     └── app/models/ 数据模型 (CanDatabase/Message/Signal + RLock)
        └── tools/cli.py  (CLI 会话)
              └── app/io/
...
| `app/models/database.py` | Web 运行时服务 | 普通类 + `__init__`，Signal 含 UUID，CanDatabase 含 `threading.RLock` + `modified` 标志 + 信号验证方法 |

两者字段相同但实现独立。`app/services/session_manager.py` 通过 `set_model_factory()` 注入 `app/models` 的 `CanDatabase` 类。
...
**运行时常驻版本**（`app/models/database.py`）：
...
- `tools/cli.py` 使用 `app/models/database.py` 的 dataclass 版本（无 UUID，无锁）
- `app/server/lifecycle.py` 使用内联类版本（有 UUID，有 RLock）
...
**决策**：`core/can_database.py` 使用 dataclass（供 CLI），`app/models/database.py` 使用普通类（供 Web 运行时）。
...
| `app/services/session_manager.py` | 会话生命周期管理（自动保存、超时清理） |
...
| `app/ws/server.py` | WebSocket 服务端（asyncio） |
| `app/ws/transport.py` | WS I/O 封装（连接管理 + 广播） |
| `app/ws/router.py` | 消息路由（type → handler 分发） |
| `app/ws/handlers/*.py` | 业务 Handler（信号/报文/文件/系统） |
...
| `frontend/src/stores/` | Pinia 状态管理（editor + ui + clipboard 等） |
